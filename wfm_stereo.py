...
class WFMStereo(gr.top_block):
    def __init__(self, freq=88.6e6, gain=40, ppm=0):
        ...
        # ── Branche B : MPX démodulé → FIFO pour redsea ──────────────
        self.fm_demod = analog.fm_demod_cf(
            channel_rate=samp_rate,
            audio_decim=mpx_decim,
            deviation=75000,
            audio_pass=65000,
            audio_stop=70000,
            gain=1.0,
            tau=0.0        # pas de déemphasis : redsea a besoin du MPX brut
        )
        ...
        self.connect(self.src, self.fm_demod, self.mult_mpx, self.f2s_mpx, self.mpx_sink)
